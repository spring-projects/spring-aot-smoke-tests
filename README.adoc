...
As the tests don't use the Spring Dependency Management Plugin, you can't use the `ext['...'] = '...'` method.
...
The version of `spring-session-jdbc` is managed by `spring-boot-dependencies`.
To allow it to be overridden, change the `enforcedPlatform` dependency on `spring-boot-dependencies` to a `platform` dependency:
...
Now, declare a constraint to control the version of `spring-session-jdbc`:
