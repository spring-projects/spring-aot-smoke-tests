...
The exact version is derived from the version in Spring Boot's dependency management using the following rules:
...
To disable this behavior set the `useSnapshots` property to `false`, as shown in the following example:

[source,]
----
./gradlew :boot:actuator-webmvc:build -PuseSnapshots=false
----
